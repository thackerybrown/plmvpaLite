function [subj] = TB_create_xvalid_indices(subj,runs_selname,varargin)

% Creates a selector for each iteration for leave-one-out cross-validation.
% NOTE: for a variant that only filters TRAINING timepoints by "actives",
% use PM_create_xvalid_indices_trainActivesOnly.m
%
...
defaults.ignore_jumbled_runs = true;
...
for r=unique(runs(runs>0))
  ...
  cursels(find(~actives)) = 0;  % only remove training trials that are not active
