function [subj] = TIB_feature_select(subj,data_patin,regsname,selsgroup,varargin)
% Originally 'feature_select' from the princeton mvpa toolbox
% modified by JR, then by AG, and then by TIB. - this variant does
% p-threshold masking
%
...
defaults.nVox_thresh = 0;%1000; %voxel count threshold
defaults.thresh = 0; %p-value threshold
...
args.new_maskstem = sprintf( ...
    '%s%s',args.new_maskstem,num2str(args.nVox_thresh));
...
for n=1:nIterations
    ...
    if args.nVox_thresh > 0 %if ~isempty(args.nVox_thresh)
    ...
    elseif args.thresh > 0
        % Now, create a new thresholded binary mask from the p-values
        % statmap pattern returned by the anova
        subj = create_thresh_mask(subj,cur_map_patname,cur_maskname,args.thresh);%,'greater_than',args.greater_than);
        subj = set_objfield(subj,'mask',cur_maskname,'group_name',args.new_maskstem);
    end
