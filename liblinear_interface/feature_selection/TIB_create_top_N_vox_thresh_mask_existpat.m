function [subj] = TIB_create_top_N_vox_thresh_mask_existpat(subj,map_patin,new_maskname,nVox_thresh,varargin)
...
for m=1:length(map_patnames)
  ...
  subj = duplicate_object(subj,'mask',subj.masks{end}.name,cur_maskname);
  ...
  display([' p-val cutoff for top ' num2str(nVox_thresh) ' features = ' num2str(sorted_pat(nVox_thresh))])
...
%   end
  ...
  subj.masks{1,end}.mat=newmat;
  %subj = set_mat(subj,'mask',cur_maskname,newmat);

  % Update the header
  hist = sprintf('%s, nfeat_thresh=%.4f, nVox=%i, created by create_nVox_thresh_mask', ...
