function [subj] = TIB_feature_select_top_N_vox_existpatmat(subj,data_patin,regsname,selsgroup,varargin)
...
for n=1:nIterations
    ...
    if ~isempty(args.nVox_thresh)
        ...
        if strcmp(args.fseltype, 'topn')
            subj = TIB_create_top_N_vox_thresh_mask_existpat(subj,cur_map_patname,cur_maskname,args.nVox_thresh);
        elseif strcmp(args.fseltype, 'rand')
             error('random voxel selection for existpatmat has not been implemented yet')
