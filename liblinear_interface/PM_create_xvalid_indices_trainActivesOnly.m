function [subj] = PM_create_xvalid_indices_trainActivesOnly(subj,runs_selname,varargin)
...
for r=unique(runs(runs>0))
  ...
  cursels(find(~actives & cursels==1)) = 0;  % only remove training trials that are not active
