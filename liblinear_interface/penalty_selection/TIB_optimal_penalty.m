...
function [opt_penalty] = TIB_optimal_penalty(choice,choice_set,v,trainpats,traintargs,in_args,cv_args)
% **Revision 1.0**
% 9/9/2014
%
...
%currently only supports AUC as the performance metric for identifying the
%best penalty. Also computes F score, simply needs to have a flag set up to
%use this if preferred. Future revisions will include this, and will
%include proportion correct (accuracy) as a performance metric as well
        ...
        for i = 1:length(in_args.penaltyRange)
            ...
            for r = 1:length(rSet)
                ...
                if strcmp(in_args.classType,'libLin')
                    ...
                    m = train(thisChoice, sparse(thisV), trainOptsOptimize);%NOTE: changed this from "ll_train(...)" which refered to mac-friendly compiled files Alan put together for Valerie
                    [theseLabels,~]=predict(choice(theseOmitted), sparse(v(theseOmitted,:)), m);%NOTE: changed this from "ll_predict(...)" which refered to mac-friendly compiled files Alan put together for Valerie
