function [subj] = PM_interpret_weights_NEW(subj,results,results_IW,S, varargin)
...
for i=1:nTimes
    ...
    for cds = 1:length(S.regNames)
        ...
        if S.generateBetaMaps
            ...
            curImp.pos.(S.regNames{cds})(Vox.pos.(S.regNames{cds}).sameSign)  = curWts{cds}(Vox.pos.(S.regNames{cds}).sameSign) .* curCanonical{cds}(Vox.pos.(S.regNames{cds}).sameSign);
            curImp.neg.(S.regNames{cds})(Vox.neg.(S.regNames{cds}).sameSign) = curWts{cds}(Vox.neg.(S.regNames{cds}).sameSign) .* curCanonical{cds}(Vox.neg.(S.regNames{cds}).sameSign);
            curImp.both.(S.regNames{cds})(Vox.both.(S.regNames{cds}).sameSign) = curImp.pos.(S.regNames{cds}) + curImp.neg.(S.regNames{cds});
            
            curImp.raw.(S.regNames{cds}) = curWts{cds}.*curCanonical{cds};
        end
...
for it = 1:length(S.impType)
    for cds = 1:length(S.regNames)
        %for j=1:nRuns
        importance.(S.impType{it}).(S.regNames{cds}).map= importance.(S.impType{it}).(S.regNames{cds}).map;
