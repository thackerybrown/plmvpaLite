function [subj] = TIB_statmap_repclustering_searchlight(subj,data_patname,regsname,selname,new_map_patname,extra_arg)
...
defaults.obj_funct = '';
defaults.adj_list = [];
defaults.add_center_voxel = false;
defaults.scratch = [];
...
for v = 1:nVox
    ...
    vpref = pat1(v); %what is categorical preference of center voxel?
    volmode = mode(sphere); %what is greatest preference of volume around center?
    
    %code center voxel based on whether its preference is predicted by
    %mode of neighbors
    if vpref == volmode
        P(v) = 1;
    else
    ...
    %code center voxel based on the "weight" (proportion) of neighbors
    %sharing MODAL preference
    W(v) = (sum(sphere == volmode))/length(sphere);
    
    
    %scratch.pat3 = pat3(vox_idx,:);
    
    scratch.v_counter = v; % store the current voxel # being operated on
...
if length(find(isnan(P)))
    warning('There are NaNs in the P map');
end

if length(find(isnan(W)))
    warning('There are NaNs in the W map');
end

%% log the binary preference match (P) and preference match weights (W) in subj
%new_map_patname_P = [data_patname '_P'];
new_map_patname_W = [data_patname '_W'];

masked_by = get_objfield(subj,'pattern',data_patname,'masked_by');
subj = initset_object(subj,'pattern',new_map_patname,P, ...
    'masked_by',masked_by);
subj = initset_object(subj,'pattern',new_map_patname_W,W, ...
    'masked_by',masked_by);

hist = sprintf('Created by %s',mfilename());
subj = add_history(subj,'pattern',new_map_patname,hist);
subj = add_history(subj,'pattern',new_map_patname_W,hist);
...
subj = add_created(subj,'pattern',new_map_patname_W,created);

%% calculate relevant states
meanP = mean(P);
meanW = mean(W);

x = P.*W; %positive prediction*weight of neighbors

Pweighted = sum(x)/sum(W); %compute a weighted prediction score for ROI (sum W*P / sum W)

subj.meanP = meanP;
subj.meanW = meanW;
subj.Pweighted = Pweighted;
