...
%basepath = ['/home/brain/host/mvpa_sample_data/' exp_name '/'];%'/Volumes/group/awagner';

%

%subj '_localizer_onsets_test'];

%mvpa_dir = [basepath study_prefix sub '/results01/'];
...
if runs_concat == 1
    ...
    if exist([S.mvpa_dir '/RSA_data/' S.subj_id '_' mask '_condensedpats.mat']);
        ...
        elseif ImgDims == 3
            ...
            %     path = [basepath study_prefix sub '/bolds/run_' run '/'];
            ...
            %%      now compute a mean pattern for __ TRs surrounding the onset; on a run-by-run basis
           % for rnum = 1:length(TRsperRun)
                

                
                for n = 1:length(names)
                %names_t = names{n}(cell2mat(runs_onsidx)==rnum);%filter to current run
                %onsets_t = onsets{n}(cell2mat(runs_onsidx)==rnum);%filter to current run
                    
                    time_idx = floor(onsets{n}/S.TR) + 1;%convert onsets to TRs
                    
                %    enoughTRs_h = (time_idx + (length(theseTRWeights)-1)) <= TRsperRun(rnum);%runlength;
                %    lengthdiff = runlength - time_idx; %calculate how many more TRs there are beyond the onset
                    %onsets{i} = sort(horzcat(onsets{i}, theseOnsets(enoughTRs_h)));
                    onsets_TR{n} = time_idx; %sort(horzcat(onsets_t{n}, onsets_t{n}{idxThisCond}(enoughTRs_h)));%put the onsets for cond{i} into an array,
                ...
                %% now integrate patterns and indices into master matrices for study-level analysis
                %rmat_condensed = [rmat_condensed _condensed_t];
                onsets_TRs = [onsets_TRs onsets_TR];
            %end
        end
    end
