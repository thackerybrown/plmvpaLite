function [res, results]= TIB_run_mvpa_xsub(subj_array, task, TRsperRun, studyName)
%Modification of TIB_run_mvpa_general - this will run an analysis that
%cross-validates over participants, not within-participant

%example call - TIB_run_mvpa_xsub({'001','003'},'CM_localizer',{[114,114],[114,114]},'8080test')
...
for b=(1:length(subj_array))
    ...
    Ss{b} = S;
    pars{b} = par;
end  % here, we loop through subs first, then concatenate

%% Concatenate subs - for the most part, one subs' info should be usable for all subs. General strategy is to concatenate info from across subs to create a new "meta subject" and then complete analysis as normal
%first, update onsets for concatenation
%compute TR counts from prior subs
TRpastbyS{1} = 0;
for b=(1:length(subj_array))
    TRpastbyS{b+1} = TRpastbyS{b} + sum(TRsperRun{b});
end
%convert TR counts to seconds
onsmodifier = cell2mat(TRpastbyS)*S.TR;
%update all onsets information
for b=(1:length(subj_array))
    for cond = 1:S.num_conds
        Ss{b}.onsets{cond} = Ss{b}.onsets{cond}+onsmodifier(b);
        Ss{b}.onsets_train_in_classifier{cond} = Ss{b}.onsets_train_in_classifier{cond}+onsmodifier(b);
        Ss{b}.onsets_test_in_classifier{cond} = Ss{b}.onsets_test_in_classifier{cond}+onsmodifier(b);
    end
end

%reset and concatenate on "meta subject" of data
S.idxOnsets_train_in_classifier = [];
S.idxOnsets_test_in_classifier = [];
S.runs_vector = [];
S.meta_runs = [];
S.num_runs(1) = 0;
S.img_files = [];
...
for cond = 1:S.num_conds
    S.onsets{cond} = [];
    S.onsets_train_in_classifier{cond} = [];
    S.onsets_test_in_classifier{cond} = [];
end

for b=(1:length(subj_array))
    for cond = 1:S.num_conds
        S.onsets{cond} = [S.onsets{cond} Ss{b}.onsets{cond}];
        S.onsets_train_in_classifier{cond} = [S.onsets_train_in_classifier{cond} Ss{b}.onsets_train_in_classifier{cond}];
        S.onsets_test_in_classifier{cond} = [S.onsets_test_in_classifier{cond} Ss{b}.onsets_test_in_classifier{cond}];
    end
    
    %concatenated idx vectors
    S.idxOnsets_train_in_classifier = [S.idxOnsets_train_in_classifier Ss{b}.idxOnsets_train_in_classifier];
    S.idxOnsets_test_in_classifier = [S.idxOnsets_test_in_classifier Ss{b}.idxOnsets_test_in_classifier];
    
    %concatenated runs vectors
    S.runs_vector = [S.runs_vector Ss{b}.runs_vector];
    S.meta_runs = [S.meta_runs Ss{b}.meta_runs];
    S.num_runs = S.num_runs+Ss{b}.num_runs;
    
    %concatenated filenames
    S.img_files = [S.img_files; Ss{b}.img_files];
    S.filenames = [S.filenames; Ss{b}.filenames];
    S.filenames_test = [S.filenames_test; Ss{b}.filenames_test];
    S.filenames_train = [S.filenames_train; Ss{b}.filenames_train];
    
    %TR durations
    S.durTrain = S.durTrain+Ss{b}.durTrain;
    S.durTest = S.durTest+Ss{b}.durTest;
