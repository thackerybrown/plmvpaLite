function []= TIB_run_mvpa(subj_array, condition1, condition2, nVox, penalty, targ_tr, TRsperRun)

% Workflow script for running binary MVPA classification on Thackery Brown's
% Circular maze navigation dataset. Requires Princeton MVPA toolbox
% functions to be located in your path.

% example usage:  TIB_run_mvpa({'cm05'},'one','two',300,1,'v1',[359 344 341 335 329 320 334 343])
...
%~~~~~~~~~TO RUN YOU WILL NEED TO CREATE:
%~~raw_filenames.mat - .mat file containing names of all functional images
%(must exist for each subject; can be created by running cellstr(SPM.xY.P) on subject's SPM.mat file)
%~~vol_info.mat - .mat file defining functional data resolution info for spm .img writing
        % To create vol_info.mat, run the following command in the matlab workspace:
        % vol_info =
        % spm_vol('name_of_image_file_with_same_dimensions_as_data_being_us
        % ed_by_the_classifier.img'); save vol_info.mat vol_info
...
for b=1:length(subj_array)  % run loop for each subject in subj array (subject ID #'s specified in short format)
    ...
    expt_name = 'localizer_shits'; %'marlieketest'; % 
    expt_dir = ['/mnt/wagner/thackery/' expt_name '/']; % top-level expt. directory where individual subject folders live
    mvpa_dir = [expt_dir '/' subj_id '/mvpa_results_blockdes']; % mvpa directory within each subject's folder (will be created below if it doesn't exist). Contains model files
    %spm_onsets_dir = [expt_dir '/' subj_id '/mvpa/']]; % directory where each subject's SPM-formatted onsets.mat files lives
    par.funcdir =[expt_dir subj_id '/bolds/']
    
    % change mask back to SEPT09_MVPA_MASK ?????
    roi_name = 'rSEPT09_MVPA_MASK_resliced4mm'%'2mmrVOTC.m';  % name of mask to be used for voxel selection (can be a small ROI, a whole-brain mask, or anywhere in between)
    ...
    for idxr = 1:length(runfolds)
        allrawfilenames{idxr,1} = dir(fullfile(par.funcdir, runfolds(idxr).name, '/swa*.nii'));%'/urun*.nii'));
    ...
    for idx = 1:length(raw_filenames)
        if length(raw_filenames{idx,1}) == 80
    ...
    raw_filenames = a(:,1);
    data_imgs_to_use = 'raw_filenames'
    
    
    num_TP_per_run = TRsperRun; % number of TRs per scanning run (coded for vector of values)
    
    load([mvpa_dir '/onsets_mel_aa.mat']); % load in your SPM-formatted onsets file (specifies onsets of each event time in seconds)
    %load([mvpa_dir '/' data_imgs_to_use]); %loads predefined cell array called raw_filenames into memory
    num_runs = numel(num_TP_per_run); %(allows script to work flexibly for subjects with missing runs)
    ...
    mvpa_workspace = [mvpa_dir '/' subj_id '_' roi_name '.mat'];
    ...
    flags.perform_second_round_of_zscoring = 0;  % z-score data again immediately prior to classification (because trial selection and TR selection undoes initial z-scoring)
    ...
    %What TRs and TR weights should we use? Note - this depends on the study,
    %and the examples below are just some of the possibilities.
    %Also note that averaging over several TRs tends to give more robust
    %pattern estimates. May benefit tasks with fewer trials. But this must be
    %balanced with a) how transient you think the effect is, and b) how well
    %isolated your event of interest is in time from other trial types
    if strcmp(targ_tr,'v1')
      ...
      TR_weights = [0.75 0.25 0 0 0]; % should sum to 1 (e.g., [0 0 .5 .5 0 0] indicates to average the 3rd and 4th post-stimulus TRs)
    elseif strcmp(targ_tr,'v2')
      TRs_to_average_over = [1 2]; 
      TR_weights = [0.5 0.5 0 0 0]; 
    elseif strcmp(targ_tr,'v3')
      TRs_to_average_over = [1 2 3]; 
      TR_weights = [0.25 0.5 0.25 0 0];        
    elseif strcmp(targ_tr,'v4')
      TRs_to_average_over = [1 2 3]; 
      TR_weights = [0.35 0.5 0.15 0 0];
    elseif strcmp(targ_tr,'v5')
      TRs_to_average_over = [1 2 3]; 
      TR_weights = [0.15 0.5 0.35 0 0];
    elseif strcmp(targ_tr,'v6')
      TRs_to_average_over = [1 2 3]; 
      TR_weights = [0.15 0.35 0.5 0 0];
    elseif strcmp(targ_tr,'v7')
       TRs_to_average_over = [1 2 3 4]; 
      TR_weights = [0 0 0.5 0.5 0 0];
    elseif strcmp(targ_tr,'v8')
       TRs_to_average_over = [1 2 3 4 5 6]; 
      TR_weights = [0 0 0.25 0.25 0.25 0.25];
    elseif strcmp(targ_tr,'v9')
       TRs_to_average_over = [1 2 3 4]; 
      TR_weights = [0 0.5 0.5 0 0];
    elseif strcmp(targ_tr,'canon')
      TRs_to_average_over = [1 2 3 4 5]; 
      TR_weights = [.0072 .2168 .3781 .2742 .1237];%attempts to mimic a model of SPM canonical haemodynamic response.
    end
...
%     traintest = {'EXP_only','CM_only','EXP>CM','EXP>CM_first_run','EXP>CM_last_run','CM>EXP'};
    ...
    if ~exist(mvpa_workspace,'file') %if a workspace .mat file has not yet been created for this subject
        ...
        [subj] = TIB_mvpa_load_and_preprocess_raw_data(subj_id, expt_name, roi_name, roi_file, raw_filenames, num_runs, num_TP_per_run);
    ...
    all_trials = sum(all_onsets,1); % vector of all trials
            ...
            %data_by_TR(6,:,:) = TR_weights(6)*subj.patterns{end}.mat(:,find(all_trials)+5); % 6th TR (10-12 sec)
...
%             5= blank
        ...
        %AA_faces = condensed_regs_all([1],:); 
        %five = condensed_regs_all([5],:); 
        
        %scr_objects = condensed_regs_all([2],:);
        %scenes = condensed_regs_all([2],:);
        %objects = condensed_regs_all([1],:); 
        %scr_objects = condensed_regs_all([2],:);
        
        AA_faces = condensed_regs_all([1],:); 
        scenes = condensed_regs_all([2],:); 
        objects = condensed_regs_all([3],:); 
...
%           end
        ...
        for n = 1: flags.num_results_iter
            ...
            for p = 1:flags.num_iter_with_same_data  % can run this loop multiple times if using a stochastic (i.e. non-deterministic) classification algorithm like backpropagation neural nets
                ...
                testo = multiple_iterations_confusion(results{1,1});
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%                 end
    ...
    for i = 1:length(results)
        a.iterations = [a.iterations results{1,i}.iterations]
    end
    ...
    if flags.generate_importance_maps == 1;
        ...
        %load([expt_dir '/' subj_id '/Masks/vol_info.mat']); %get functional data resolution info for spm .img writing
        % To create vol_info.mat, run the following command in the matlab workspace:
        ...
        vol_info = spm_vol(roi_file); %create vol_info on the fly
