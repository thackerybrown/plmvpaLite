...
function [model] = classifierLogisticRegressionSimple_inlined( varargin )
...
W = zeros(nFeatures+1,nClasses);  % changed by JR to make function deterministic (classification outcome will always be the same, given identical parameters)

switch optimizationMethod
    case {'minimize'}
        % use carl rasmussen's minimize.m (renamed to optimize.m to avoid a clash with CVX, appended below)
        [Wvector,Wvalue] = optimize(W(:),'classifierLogisticRegressionSimple_gradients',1000,examples,labels,lambda,indices,delta,regularization,'minimize');
end
...
optimization = varargin{7};


[n,m] = size(X_new); k = size(ymask,2); tm = m + 1; W = reshape(Wvectorized,[tm k]); df = zeros(tm,k);

df(1,:) = sum(ymask,1); for j = 1:k; df(2:end,j) = sum(X_new(yindices{j},:),1)'; end

tmp1 = X_new * W(2:end,:) + repmat(W(1,:),n,1);
tmp2 = exp(tmp1); % n x k
tmp3 = tmp2 ./ repmat(sum(tmp2,2),1,k); % n x k

df(1,:) = df(1,:) - sum(tmp3,1); for j = 1:k; df(2:end,j) = df(2:end,j) - sum(X_new .* repmat(tmp3(:,j),[1,m]),1)';end

f_new = sum( sum(tmp1.*ymask,2) - log(sum(tmp2,2)) );

tmp = W(2:end,:);
f_new  = f_new - 0.5*lambda*sum(tmp(:).^2);
df(2:end,:) = df(2:end,:)-lambda*tmp;

df = df(:); f_new  = -1*f_new; df = -1*df;
