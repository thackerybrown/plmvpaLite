function []= JR_run_mvpa_v9(subj_array, condition1, condition2)
...
for b=subj_array  % allows subject ID #'s to be specified in short format
    ...
    flags.anova_nVox_thresh =12000; % alternative to specifying p-value threshold; uses top N voxels (0 = DON'T PERFORM ANY FEATURE SELECTION)
    ...
    flags.artdetect_motion_thresh = 5; % specify ArtDetect bin for motion outliers
    flags.artdetect_global_signal_thresh = 5; % specify ArtDetect bin for global signal outliers
    flags.remove_outlier_trials = 5;  % on-the-fly outlier detection/removal; specify how many std dev from whole brain mean to exclude as outliers (0 = don't exclude any trials)
    ...
    TR_weights = [0 0 1 1 0 0]; % should sum to 1
    ...
    if flags.optimize_penalty_param == 1
        dir_str = [condition1 '_vs_' condition2 '_' class_args.train_funct_name(6:end) '_OPTIMAL_pen_' num2str(flags.anova_nVox_thresh) 'vox'];
    else
        dir_str = [condition1 '_vs_' condition2 '_' class_args.train_funct_name(6:end) '_pen' num2str(class_args.penalty) '_' num2str(flags.anova_nVox_thresh) 'vox_aggressive_outlier_removal'];
    end
    
    if flags.generate_importance_maps == 1        
        importance_maps_dir=[expt_dir '/mvpa_results/improved_importance_maps/' dir_str];
    ...
    if flags.write_data_log_to_text_file == 1        
        xls_results_data_logs_dir=[expt_dir '/mvpa_results/NEW_MVPA_MASK/' dir_str];
    ...
    if flags.write_data_log_to_text_file==1
        ...
        fprintf(fid, '#trials\t\t\t\t\t');
        fprintf(fid, '%3.0f\t', mean(data_log.number_of_trials_per_bin,1));
        fprintf(fid, '\n');
    ...
    keep b subj_array condition1 condition2
