function [subj] = JR_statmap_searchlight(subj,data_patname,regsname,selname,new_map_patname,extra_arg)
...
%map = nan(nVox,1);
 %JR mod:  initialize acts_vector & corrects vector
acts_vector = single(zeros(1,size(xval3_idx,2))); 
corrects_vector = logical(zeros(1,size(xval3_idx,2)));
...
if exist('pat3','var')
    ...
    for v = 1:nVox
        ...
        [acts_vector corrects_vector scratch] = funct_h(sphere, regs1, scratch);
        
        % concatenate the activation and corrects vectors across iterations
        if args.cur_iteration == 1
            subj.acts_vector{v} = acts_vector;
            subj.corrects_vector{v} = corrects_vector;
        else
            
            if ~isfield(subj,'acts_vector') || length(subj.acts_vector)<v
                subj.acts_vector{v} = [];
            end
            if ~isfield(subj,'corrects_vector') || length(subj.corrects_vector)<v
                subj.corrects_vector{v} = [];
            end
            
            subj.acts_vector{v} = horzcat(subj.acts_vector{v}, acts_vector);
            subj.corrects_vector{v} = horzcat(subj.corrects_vector{v}, corrects_vector);
        end
