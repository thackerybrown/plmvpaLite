function []= JR_run_mvpa_v6_ROC(subj_array, condition1, condition2, balance_per_subj_bin);
...
for b=subj_array
    ...
    mvpa_workspace = [PAST_dir '/' subj_id '/mvpa/' subj_id '_merged_AAL_ROIs_FIXED_HOLES_s4mm.mat'];

    num_results_iter = 1; % number of times to run the cross validation process
    num_iterations_with_same_classifier = 3;
    ...
    %ROIs_to_use = [59 60 61 62 3 4 77 78]
...
%                 ylabel('P(Old|Old)')
    ...
    save_cmd = ['save ' subj_id '_ROI_ROC_data.mat ROC'];
    eval(save_cmd);
    clear ROC;
