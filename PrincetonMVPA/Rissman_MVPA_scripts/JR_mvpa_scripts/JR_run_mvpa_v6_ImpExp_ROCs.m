...
for b=subj_array
    ...
    for n = 1: num_results_iter
        ...
        %%%  NOTE:  HARD-CODED 1016:END IN ORDER TO MAKE SURE THAT
        %%%  RESPONSES FROM THE FIRST 5 IMPLICIT RUNS DON'T GET EXCLUDED BY
        %%%  IN THE SUBJECTS WHO HAVE FAKE PLACEHOLDER ONSETS IN THEIR
        %%%  EXPLICIT DATA
        
        % SPECIAL FIX because reg #6 (NEW_recollect) is a fake placeholder trial for some subjects
        if ~isempty(find(strcmp(subj_id,{'s203','s204','s205','s207','s208'})))
            all_regs(6,1016:end)=0;
        end
        
        % SPECIAL FIX because reg #7 (NEW_HC_old) is a fake placeholder trial for some subjects
        if ~isempty(find(strcmp(subj_id,{'s207'})))
            all_regs(7,1016:end)=0;
        end
        
        % SPECIAL FIX because reg #5 (OLD_HC_new) is a fake placeholder trial for some subjects
        if ~isempty(find(strcmp(subj_id,{'s205','s207'})))
            all_regs(5,1016:end)=0;
        end
    ...
    keep b subj_array condition1 condition2 which_traintest
end
