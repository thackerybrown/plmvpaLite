function []= JR_run_mvpa_v10(subj_array, condition1, condition2, nVox)
...
for b=subj_array  % allows subject ID #'s to be specified in short format
    ...
    mvpa_workspace = [expt_dir '/' subj_id '/mvpa/' subj_id '_NEW_MVPA_MASK_wa.mat'];
    ...
    if flags.optimize_penalty_param == 1
        dir_str = ['UNSMOOTHED_' condition1 '_vs_' condition2 '_' class_args.train_funct_name(6:end) '_OPTIMAL_pen_' num2str(flags.anova_nVox_thresh) 'vox'];
    else
        dir_str = ['UNSMOOTHED_' condition1 '_vs_' condition2 '_' class_args.train_funct_name(6:end) '_pen' num2str(class_args.penalty) '_' num2str(flags.anova_nVox_thresh) 'vox'];
    end
    
    if flags.generate_importance_maps == 1        
        importance_maps_dir=[expt_dir '/mvpa_results/FINAL_IMP_MAPS/' dir_str];
    ...
    if flags.write_data_log_to_text_file == 1        
        xls_results_data_logs_dir=[expt_dir '/mvpa_results/FINAL_CLASS_PERF/' dir_str];
    ...
    if ~isempty(find(strcmp(subj_id,{'118','s205','s207'})))
        all_regs(5,:)=0;
    end
...
%         end
    ...
    if flags.write_data_log_to_text_file==1
        ...
        fprintf(fid, '%4.2f\t', reshape(mean(data_log.class_counts_by_quartile_rank,3)',1,8));
        fprintf(fid, '\n');
        fclose(fid);
    end
