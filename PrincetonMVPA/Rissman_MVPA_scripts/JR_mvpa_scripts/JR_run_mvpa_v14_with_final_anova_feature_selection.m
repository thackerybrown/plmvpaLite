function []= JR_run_mvpa_v14_with_final_anova_feature_selection(subj_array, condition1, condition2, nVox)
...
for b=subj_array  % allows subject ID #'s to be specified in short format
    ...
    if flags.write_data_log_to_text_file == 1        
        xls_results_data_logs_txt_dir=[expt_dir '/mvpa_results/FINAL_ROC_data/feature_selections_test/FINAL_CLASS_PERF/' dir_str];
    ...
    if flags.save_data_log_as_mat_file == 1
        xls_results_data_logs_mat_dir=[expt_dir '/mvpa_results/FINAL_ROC_data/feature_selections_test/data_logs/' dir_str];
...
%             active_trials = new_active_trials;
        ...
        if flags.anova_p_thresh ~= 1
            ...
            subj = JR_feature_select(subj,'spiral_d_hp_z_condensed','conds','runs_final_xval','thresh',flags.anova_p_thresh, 'statmap_funct','statmap_anova','statmap_arg',statmap_arg);
