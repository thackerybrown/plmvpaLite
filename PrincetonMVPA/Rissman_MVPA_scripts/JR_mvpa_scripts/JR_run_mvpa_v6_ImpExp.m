function []= JR_run_mvpa_v6_ImpExp(subj_array, condition1, condition2, which_traintest);
...
%     class_args.penalty = 1;
    ...
    if flags.generate_importance_maps == 1
        importance_maps_dir=[expt_dir '/mvpa_results/IMPLICIT_EXPLICIT_IMP_MAPS/' dir_str];
    ...
    if flags.write_data_log_to_text_file == 1
        xls_results_data_logs_dir=[expt_dir '/mvpa_results/IMPLICIT_EXPLICIT_CLASS_PERF_2nd_zscoring/' dir_str];
...
%             Objective_old(find(subj.selectors{1}.mat>5))=FAs(find(subj.selectors{1}.mat>5));
%             Objective_new(find(subj.selectors{1}.mat>5))=CRs(find(subj.selectors{1}.mat>5));
             FAs(find(subj.selectors{1}.mat>5))=Hits(find(subj.selectors{1}.mat>5));
