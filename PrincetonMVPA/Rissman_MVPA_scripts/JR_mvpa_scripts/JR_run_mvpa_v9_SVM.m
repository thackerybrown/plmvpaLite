function []= JR_run_mvpa_v9_SVM(subj_array, condition1, condition2)
...
for b=subj_array  % allows subject ID #'s to be specified in short format
    ...
    importance_maps_dir=[expt_dir '/mvpa_results/improved_importance_maps/' condition1 '_vs_' condition2 '__balanced_SVM_3000vox'];
    xls_results_data_logs_dir=[expt_dir '/mvpa_results/feature_selection_tests/' condition1 '_vs_' condition2 '_balanced_SVM_3000vox'];
    ...
    flags.anova_nVox_thresh = 3000; % alternative to specifying p-value threshold; uses top N voxels (0 = DON'T PERFORM ANY FEATURE SELECTION)
    ...
    %TR_weights = [.0072 .2168 .3781 .2742 .1237];  % from SPM canonical
...
%subj.patterns{end}.mat = single(subj.patterns{end}.mat);  % save RAM
...
%             
%             if x == 0 % find optimal averaged penalty first time only (to save time)
%                 [subj best_penalties penalty_iteration_results] = optimal_logreg_penalty(subj,'spiral_d_hp_z_condensed','conds','runs_final_xval','runs_final',classifier_mask,'conditions_of_interest_final','use_iteration_perf',false,'perform_final_classification',false)
            ...
            for p = 1:flags.num_iter_with_same_data
                ...
                if flags.display_performance_breakdown == 1
                    ...
                    for a = 1:num_runs
                        ...
                        acts_diff_vector = horzcat(acts_diff_vector, results{x}.iterations(a).scratchpad.predictions');
                    end
...
%                    data_log.penalty_param(x) = class_args.penalty;
    ...
    if flags.write_data_log_to_text_file==1
        ...
        fprintf(fid, 'results_iter\toverall_acc\tHits\tFAs\td_prime\tOLD_recollect\tOLD_hc_old\tOLD_lc_old\tOLD_lc_new\tOLD_hc_new\tNEW_recollect\tNEW_hc_old\tNEW_lc_old\tNEW_lc_new\tNEW_hc_new\tTop50\tTop100\tTop150\tTop200\tTop250\tTop300\tTop350\tTop400\tTop25pct\tTop50pct\tTop75pct\tPenalty_param\n');
        fprintf(fid, '#trials\t\t\t\t\t');
        ...
        for q=1:x
          ...
          %  fprintf(fid, '%4.4f\t', data_log.penalty_param(q));
            fprintf(fid, '\n');
