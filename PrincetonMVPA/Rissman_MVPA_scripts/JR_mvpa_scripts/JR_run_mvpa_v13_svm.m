function []= JR_run_mvpa_v13_svm(subj_array, condition1, condition2, nVox)
...
for b=subj_array  % allows subject ID #'s to be specified in short format
    ...
    flags.num_results_iter = 10; % number of times to run the entire classification process (select subset of the data and train/test classifier)
    ...
    if flags.optimize_penalty_param == 1
        ...
        dir_str = ['BIAS_SCREENED_BAL_RUNS_' condition1 '_vs_' condition2 '_' class_args.train_funct_name(6:end) 'pen' num2str(class_args.penalty) '_' num2str(flags.anova_nVox_thresh) 'vox'];
    end
