function [subj] = JR_zero_mean_center_runs(subj,patname,selname,varargin)
...
defaults.use_mvpa_ver = true;  % JR edit:  always use mvpa ver
...
for r = 1:nRuns
  ...
  if sum(this_run)~=sum(actives_this_run)
    ...
    pat(:,this_run) = (pat(:,this_run)-mu); % JR edit:  removed the denominator (./sigma);
