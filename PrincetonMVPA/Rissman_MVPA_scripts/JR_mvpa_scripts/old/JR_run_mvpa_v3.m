...
save_workspace = 1; % 1 = yes please, 0 = no thanks
...
equate_number_of_trials_per_bin = 0;
equate_subjective_status = 0;
plot_mean_timecourses = 0;
display_performance_breakdown = 1;
generate_importance_maps = 1;


condnames =  {'subjective_old_hc_only','subjective_new'}; % specify which conditions to use for classification (must correspond to the names of conditions specified below)
...
if plot_mean_timecourses == 1 % plot mean timecourses for whole ROI (useful check of data quality & overall activity pattern)
...
if anova_p_thresh ~= 1 % run feature selection ANOVA
...
for x = 1: num_results_iter
    ...
    if display_performance_breakdown == 1
...
time2finish = toc/60;
disp(['Finished in ' num2str(time2finish) ' minutes']);