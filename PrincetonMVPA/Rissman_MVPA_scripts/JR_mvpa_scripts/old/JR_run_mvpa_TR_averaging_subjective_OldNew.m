...
roi_filename =  'F>35_mask.img'; %'L_parietal_mem_str_p05.nii'; ;%; %'all_hits-cr_p01.nii'; %'new>old_t>3.img';
anova_p_thresh = .005;
...
old = sum(all_regs([1 2 6 7],:));  %all recollect and HC old responses
new = sum(all_regs([5 10],:)); %all HC new responses


%old = sum(all_regs([1 2],:));  %recollect and HC old hits
%new = all_regs(10,:); %high conf CRs
...
t0 = subj.patterns{4}.mat(:,find(all_trials)-1); % -1 TR (-2-0 sec)
t1 = subj.patterns{4}.mat(:,find(all_trials)+0); % 1st TR (0-2 sec)
t2 = subj.patterns{4}.mat(:,find(all_trials)+1); % 2nd TR (2-4 sec)
t3 = subj.patterns{4}.mat(:,find(all_trials)+2); % 3rd TR (4-6 sec)
t4 = subj.patterns{4}.mat(:,find(all_trials)+3); % 4th TR (6-8 sec)
t5 = subj.patterns{4}.mat(:,find(all_trials)+3); % 5th TR (8-10 sec)

mean_data = (t3+t4+t5)/3;

mean_baseline = (t0+t1)/2;

mean_data = mean_data - mean_baseline; %% BASELINE CORRECT MEAN DATA
...
% run classifier (No hidden layer NN Toolbox backprop algorithm)
 ...
 for j = 1:10
     old_resp(j)=mean(results.iterations(j).perfmet.corrects(find(results.iterations(j).perfmet.desireds == 1)));
     new_resp(j)=mean(results.iterations(j).perfmet.corrects(find(results.iterations(j).perfmet.desireds == 2)));
 end
 old_resp
 new_resp
 mean(old_resp)
