...
subj_id = 's102';
exp_name = 'PAST';
roi_filename = 'whole_brain.img' %'F>35_mask.img';
...
% define composite conditions of interest (by merging individual conditions)
...
for i = 1: size(regs,2)
    if ~isempty(find(regs(:,i)))
        ...
        all_regs_condensed(:,trial_counter) = all_regs(:,i);  % make a condensed version of all regs to use for identifying performance on sub-conditions
...
% run classifier (No hidden layer NN Toolbox backprop algorithm)
 ...
 % output performance values for each run for old and new stims separately
 for j = 1:num_runs
