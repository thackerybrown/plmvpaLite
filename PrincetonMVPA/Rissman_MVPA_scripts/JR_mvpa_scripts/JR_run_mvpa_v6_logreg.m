function []= JR_run_mvpa_v6_logreg(subj_array, condition1, condition2, balance_per_subj_bin);
...
for b=subj_array
    ...
    importance_maps_dir=[PAST_dir '/mvpa_results/improved_importance_maps/' condition1 '_vs_' condition2 '_' balanced_or_unbal '_FIXED_AAL_8mm_logreg_top5000'];
    xls_results_data_logs_dir=[PAST_dir '/mvpa_results/xls_results_data_logs/' condition1 '_vs_' condition2 '_' balanced_or_unbal '_FIXED_AAL_8mm_logreg_top5000'];
    ...
    for n = 1: num_results_iter
        ...
        if exist('anova_nVox_thresh','var')
            subj = JR_feature_select_top_N_vox(subj,'spiral_d_hp_z_condensed','conds','runs_xval','nVox_thresh',anova_nVox_thresh,'statmap_funct','statmap_anova','statmap_arg',statmap_arg);
            %subj = JR_feature_select_top_N_vox_iterative(subj,'spiral_d_hp_z_condensed','conds','runs_xval','nVox_thresh',anova_nVox_thresh,'statmap_funct','statmap_anova','statmap_arg',statmap_arg);
            %JR_feature_select_top_N_vox_iterative(subj,'spiral_d_hp_z_condensed','conds','runs_xval','nVox_thresh',flags.anova_p_thresh, 'statmap_funct','statmap_RFE');
            classifier_mask = subj.masks{end}.group_name; % use group of masks created by ANOVA
        else
            classifier_mask = subj.masks{1}.name; % use original mask
        end
