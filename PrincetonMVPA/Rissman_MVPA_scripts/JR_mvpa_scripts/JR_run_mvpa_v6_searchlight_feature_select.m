function [subj results] = JR_run_mvpa_v6_searchlight_feature_select(subj_arr, cond1, cond2, balance_per_subj_bin)
...
for b=subj_arr
 ...
 load(['/Users/Jesse/fMRI/data/PAST/behavioral/FACE_expt/data/' subj_id '/onsets.mat']);
    ...
    for x = 1: num_results_iter
        ...
        flags.remove_outlier_trials = 5;  % how many std dev from whole brain mean to exclude as outliers (0 = don't exclude any trials)
        ...
        if flags.remove_artdetect_outliers == 1
            ...
            m_outliers = movement_outlier_trials{5};  % remove trials with more than .3mm/TR of movement
            gs_outliers = global_signal_outlier_trials{5}; % remove trials with global signal change of +/- 3 SD from mean
        ...
        subj = create_sorted_mask( ...
            ...
            'spiral_d_hp_z_condensed_srch_4000',4000, ...
            'descending',true);
        ...
        classifier_mask = 'spiral_d_hp_z_condensed_srch_4000'; % use original mask
