function []= JR_run_mvpa_v11(subj_array, condition1, condition2, nVox)
...
for b=subj_array  % allows subject ID #'s to be specified in short format
    ...
    if flags.generate_importance_maps == 1        
        importance_maps_dir=[expt_dir '/mvpa_results/ROC_data/FINAL_IMP_MAPS/' dir_str];
    ...
    if flags.write_data_log_to_text_file == 1        
        xls_results_data_logs_txt_dir=[expt_dir '/mvpa_results/ROC_data/FINAL_CLASS_PERF/' dir_str];
    ...
    if flags.save_data_log_as_mat_file == 1
        xls_results_data_logs_mat_dir=[expt_dir '/mvpa_results/ROC_data/data_logs/' dir_str];
