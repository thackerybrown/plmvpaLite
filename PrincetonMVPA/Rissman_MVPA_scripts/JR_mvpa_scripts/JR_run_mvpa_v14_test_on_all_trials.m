function []= JR_run_mvpa_v14_test_on_all_trials(subj_array, condition1, condition2, nVox)
...
for b=subj_array  % allows subject ID #'s to be specified in short format
    ...
    if flags.optimize_penalty_param == 1
        ...
        dir_str = ['TRAIN_' condition1 '_vs_' condition2 '_TEST_on_all_conds_' class_args.train_funct_name(6:end) 'pen' num2str(class_args.penalty) '_' num2str(flags.anova_nVox_thresh) 'vox'];
    end
...
%             
%             %% At this point, the testing selectors are balanced (num Cond A = num Cond B) within
%             %% each run, but the training selectors are balanced across the
%             %% entire set of training runs, rather than within each run.
%             %% Here we will correct this by hand, using the testing
%             %% selectors of each fold to guide which trials from the other
%             %% folds get used during training
%             
%             % First, compile a vector that indexes all test trials (2's in the
%             % x-valid selectors)
%             
...
%             end
%                     
%             % Now, modify each of the x-valid sectors accordingly to only
%             % use these trials
%             
%             for kk = 1:num_runs
%                 all_training_timepoints = find(subj.selectors{1}.mat~=kk);
%                 subj.selectors{end-num_runs+kk}.mat(all_training_timepoints)=0; % first set all training timepoint to zeros
%                 new_training_set_indices = intersect(compiled_test_indices, all_training_timepoints);
%                 subj.selectors{end-num_runs+kk}.mat(new_training_set_indices)=1; % now, set those in test_indices to one
%             end
%                   
%             
%             
...
%             end
          ...
          %%  Within each x-valid fold, reactivate all test patterns
          
          temp_vector = zeros(1, length(Objective_old));
          temp_vector(active_trials)=1;
          temp_vector(combined_outliers)=[];             
          original_active_trials = find(temp_vector); % save a copy of the original active_trials vector
          
          active_trials = [1:length(Objective_old)]; % make a new vector for the active trials (e.g., [1:400]);
          active_trials(combined_outliers)=[]; % remove any outlier trials
          
          
          for rr = 1:num_runs
              run_inds = intersect(find(subj.selectors{end-num_runs-1}.mat==rr), active_trials); % get the trial indices of the trials within "run rr"
              subj.selectors{end-num_runs+rr}.mat(run_inds)=2;
              subj.selectors{end-num_runs+rr}.mat(combined_outliers)=0;
            
           end
          
%             subj.regressors{end}.mat(1,:) = Objective_old;
%               subj.regressors{end}.mat(2,:) = Objective_new;
            ...
            for p = 1:flags.num_iter_with_same_data
               ...
               % [subj results{x}] = cross_validation(subj,'spiral_d_hp_z_condensed','conds_scrambled','runs_final_xval_bal',classifier_mask,class_args);
                ...
                if flags.display_performance_breakdown == 1
                    ...
                    % create a new desireds_vector that codes for Objective Old (1) and Objectively New (2) items
                    desireds_vector = zeros(1,length(active_trials));
                    desireds_vector(Objective_old(active_trials)==1)=1;
                    desireds_vector(Objective_new(active_trials)==1)=2;
                    ...
                    for a = 1:num_runs
                        ...
                        acts_condA_vector = horzcat(acts_condA_vector, results{x}.iterations(a).acts(1,:));
                        acts_diff_vector = horzcat(acts_diff_vector, results{x}.iterations(a).acts(1,:)-results{x}.iterations(a).acts(2,:));
                    end

                    correct_vector = zeros(1,length(active_trials));
                    correct_vector(guesses_vector==desireds_vector)=1;
                    
                    
                     overall_accuracy = mean(correct_vector(original_active_trials))
                     overall_hit_rate = mean(correct_vector(intersect(original_active_trials,find(desireds_vector==1))));
                     overall_fa_rate = 1-mean(correct_vector(intersect(original_active_trials,find(desireds_vector==2))));
                    ...
                    for b = 1:10
                        ...
                        mean_condA_acts_by_resp(b) = mean(acts_condA_vector(find(condensed_regs_all(b,active_trials))));
...
% 
%                     acc_percentiles(1)=mean(abs_correct_sorted(1:ceil(num_trials(x)*1.0)));
%                     acc_percentiles(2)=mean(abs_correct_sorted(1:ceil(num_trials(x)*.75)));
%                     acc_percentiles(3)=mean(abs_correct_sorted(1:ceil(num_trials(x)*.50)));
%                     acc_percentiles(4)=mean(abs_correct_sorted(1:ceil(num_trials(x)*.25)));
                    ...
                    for acc_bin = 1:20
                        ...
                        hit_percentiles(acc_bin) = mean(abs_correct_sorted(intersect(included_trial_inds,class_A_inds)));
                        fa_percentiles(acc_bin) = 1- mean(abs_correct_sorted(intersect(included_trial_inds,class_B_inds)));
                    end
                    ...
                    hit_percentiles(isnan(hit_percentiles))=0;
                    fa_percentiles(isnan(fa_percentiles))=0;
                    ...
                    for b = 1:10
                        number_of_trials_per_bin_by_quartile_rank(1,b) = length(find(condensed_regs_all(b,active_trials(abs_ind(1:ceil(num_trials(x)*1.0))))));
                        number_of_trials_per_bin_by_quartile_rank(2,b) = length(find(condensed_regs_all(b,active_trials(abs_ind(1:ceil(num_trials(x)*.75))))));
                        number_of_trials_per_bin_by_quartile_rank(3,b) = length(find(condensed_regs_all(b,active_trials(abs_ind(1:ceil(num_trials(x)*.50))))));
                        number_of_trials_per_bin_by_quartile_rank(4,b) = length(find(condensed_regs_all(b,active_trials(abs_ind(1:ceil(num_trials(x)*.25))))));                        
                    end
...
%                     ylabel('P(Old|Old)')
                ...
                if flags.write_data_log_to_text_file==1
                    ...
                    data_log.hit_percentiles(x,:) = hit_percentiles;
                    data_log.fa_percentiles(x,:) = fa_percentiles;                    
    ...
    if flags.write_data_log_to_text_file==1
        ...
        for q=1:x
            ...
            fprintf(fid, '%4.0f\t', (num_trials(q)/2));
             fprintf(fid, '%4.4f\t', data_log.classification_accuracy_by_resp(q,:));
            fprintf(fid, '%4.4f\t', data_log.number_trials_per_bin(q,:));
...
%              fprintf(fid, '%4.4f\t', data_log.fa_percentiles(q,:));
        ...
        fprintf(fid, '%4.4f\t', mean(data_log.classification_accuracy_by_resp,1));
        fprintf(fid, '%4.4f\t', mean(data_log.number_trials_per_bin,1));
