function [subj best_penalties penalty_iteration_results results] = optimal_ridge_penalty(subj,patin,regsname,selgroup,runs_selname,mask,actives_selname,varargin)

% Determines effective ridge regression penalties from training data and
% applies the best penalty in a ridge classification.
%
% [SUBJ BEST_PENALTIES PENALTY_ITERATION_RESULTS RESULTS] = OPTIMAL_RIDGE_PENALTY(SUBJ,PATIN,REGSNAME,SELGROUP,RUNS_SELNAME,MASK,ACTIVES_SELNAME...)
%
% This script first applies a broad range of penalties (0 0.01 0.1 1.0 10
% 100 1000 10000) to ridge regression classifications on training data,
% then conducts a narrower targeted search around the penalty with the
% highest performance, and finally runs a ridge classification on test data
...
%   not set to 'none').
%
% PATIN is the dataset that will be fed into the ridge regression
...
% ridge penalty search if CREATE_NONPEEKING_SELECTOR is switched to false.
%
...
%
% PENALTY_SELNAME(optional,default = {'ridge_selector'}). Specifies the
...
defaults.penalty_selname = 'ridge_selector';
...
if args.create_nonpeeking_selector == true
    ...
    for i=1:max(runs_selname)
        ridge_selector{i}=ones(1,length(runs_selname));
        for j=1:length(runs_selname)
            if runs_selname(j)==i
                ridge_selector{i}(j)=0;
            end
        end
        if i == max(runs_selname)
            for j=1:length(runs_selname)
                if runs_selname(j)==1
                    ridge_selector{i}(j)=2;
                end
            end
        else
            for j=1:length(runs_selname)
    ...
    if strcmp(actives_selname,'none') == 0
        for i=1:length(ridge_selector)
            ridge_selector{i} = ridge_selector{i}.* get_mat(subj,'selector',actives_selname);
        end
    end
    ...
    % details:
    for i=1:length(ridge_selector)
        subj=initset_object(subj,'selector',sprintf('%s_%i',args.penalty_selname,i),ridge_selector{i},'group_name',args.penalty_selname);

        sprintf('Created %s called %s','selector',sprintf('%s_%i',args.penalty_selname,i))
        created.function = 'optimal_ridge_penalty';
        ...
        object_hist = sprintf('Created by optimal_ridge_penalty for nonpeeking evaluation of penalties - iteration #%i',i);
    ...
    if strcmp(actives_selname,'none') == 0
        for i = 1:length(nonpeeking_selector_names)
            ...
            object_hist = sprintf('Created by optimal_ridge_penalty - iteration #%i',i);
...
% Conduct the first pass of trying different ridge penalties:

% Run through ridge classification on a wide range of penalty values:
broad_range = [0 0.01 0.1 1.0 10 100 1000 10000];
for ridge_pen = broad_range

    % Keep track of which penalties have been tried:
    penalties = [penalties ridge_pen];

    class_args.train_funct_name = 'train_ridge';
    class_args.test_funct_name = 'test_ridge';
    class_args.penalty = ridge_pen;
...
if args.use_iteration_perf == false
    ...
    if whichmax == 1
        for ridge_pen = penalty_results(1,whichmax):(penalty_results(1,(whichmax+1))/2)/(args.npenalties-1):penalty_results(1,(whichmax+1))/2
    ...
    elseif whichmax == length(penalties)
        for ridge_pen = penalty_results(1,(whichmax))/2:((penalty_results(1,whichmax)*10)/2-penalty_results(1,(whichmax))/2)/(args.npenalties-1):(penalty_results(1,whichmax))*10/2
        ...
        for ridge_pen = penalty_results(1,(whichmax))/2:(penalty_results(1,(whichmax+1))/2-penalty_results(1,(whichmax))/2)/(args.npenalties-1):penalty_results(1,(whichmax+1))/2
...
if args.use_iteration_perf == false
    ...
    % Create a plot of accuracy against ridge penalty:
    ...
    title('Classification performance (non-peeking) for ridge penalties' );
...
% If the user hasn't declined this option, perform a final ridge
...
if args.perform_final_classification == true
    ...
    if args.use_iteration_perf == false
        ...
        disp(sprintf('\nCross-validation with ridge regression using a searched for penalty of %i:',best_penalties));
        for i = 1:max(runs_selname)
        ...
        % Report the results to the command window:
        disp(sprintf('\nCross-validation with ridge regression using searched for penalties:'));
        for i = 1:max(runs_selname)
            disp(sprintf('\t%i\t%.2f\t(ridge penalty = %i)',i,results.iterations(i).perf,best_penalties(i)) );
        end
        disp(sprintf('\n\tTotal\t%.2f',results.total_perf) );
    end
