function []= JR_run_mvpa_v14_scrambled(subj_array, condition1, condition2, nVox)
...
for b=subj_array  % allows subject ID #'s to be specified in short format
    ...
    if flags.optimize_penalty_param == 1
        dir_str = ['SCRAMBLED_2_' condition1 '_vs_' condition2 '_' class_args.train_funct_name(6:end) 'OPTIMAL_pen_' num2str(flags.anova_nVox_thresh) 'vox'];
    else
        dir_str = ['SCRAMBLED_2_' condition1 '_vs_' condition2 '_' class_args.train_funct_name(6:end) 'pen' num2str(class_args.penalty) '_' num2str(flags.anova_nVox_thresh) 'vox'];
    end
...
%             end
           ...
           [subj] =  JR_scramble_regressors(subj,'conds','runs_final','conditions_of_interest_final','conds_scrambled');
            ...
            for p = 1:flags.num_iter_with_same_data
               ...
               % [subj results{x}] = cross_validation(subj,'spiral_d_hp_z_condensed','conds','runs_final_xval',classifier_mask,class_args);
                [subj results{x}] = cross_validation(subj,'spiral_d_hp_z_condensed','conds_scrambled','runs_final_xval',classifier_mask,class_args);
