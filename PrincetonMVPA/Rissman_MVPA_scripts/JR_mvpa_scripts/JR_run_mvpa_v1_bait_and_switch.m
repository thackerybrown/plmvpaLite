...
if ~exist('mvpa_workspace')
    ...
    save_cmd = ['save ' subj_id '_spiral_d_hp_z_' roi_file(:,end-4) '_' mvpa_datetime '.mat'];
    eval(save_cmd);
...
if generate_importance_maps == 1;  % NO ANOVA VERSION
    ...
    vol_info.fname = [condnames_bait{1} '_no_anova_' mvpa_datetime '.img'];
    spm_write_vol(vol_info,immap1);
    vol_info.fname = [condnames_bait{2} '_no_anova_' mvpa_datetime '.img'];
