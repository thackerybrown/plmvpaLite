function [subj results] = JR_run_mvpa_v4(mvpa_workspace)
...
if ~exist('mvpa_workspace')
    ...
    subj_id = 's102';
    exp_name = 'PAST';
    roi_file = '/Users/Jesse/fMRI/data/PAST/fMRI/s102/mvpa/whole_brain_no_L_motor.img'; %'/Users/Jesse/fMRI/data/PAST/fMRI/4x4x4/merged_AAL_ROIs.nii';
    roi_name = 'whole_brain_no_L_motor' %'merged_AAL_ROIs'
    num_runs = 10;
    num_TP_per_run = 203;

    % load user-created filename and onsets lists into workspace
    load('raw_filenames_sra.mat') %load('raw_filenames_s8mm_wa.mat')
    ...
    if flags.save_workspace == 1
        save_cmd = ['save ' subj_id '_' roi_name '_6mm_smoothing_Native_Space_' mvpa_datetime '.mat'];
...
% This seemed like a good idea, but I've commented it out below b/c seems to occassionally hurt performance
