function []= JR_run_mvpa_v14(subj_array, condition1, condition2, roi_name)
...
for b=subj_array  % allows subject ID #'s to be specified in short format
    ...
    mvpa_workspace = [expt_dir '/' subj_id '/mvpa/' subj_id '_' roi_name '_wa.mat'];
    ...
    if flags.optimize_penalty_param == 1
        dir_str = ['5bin_ROCs_' roi_name '_' condition1 '_vs_' condition2 '_' class_args.train_funct_name(6:end) 'OPTIMAL_pen_' num2str(flags.anova_nVox_thresh) 'vox'];
    else
        dir_str = ['5bin_ROCs_' roi_name '_' condition1 '_vs_' condition2 '_' class_args.train_funct_name(6:end) 'pen' num2str(class_args.penalty) '_' num2str(flags.anova_nVox_thresh) 'vox'];
    end
...
%                     acc_percentiles(4)=mean(abs_correct_sorted(1:ceil(num_trials(x)*.25)));
                ...
                if flags.plot_ROC_curve == 1
                    ...
                    bin_size = ceil(length(desireds_sorted)/5);
                    j = 0;
                    for i = [bin_size bin_size*2 bin_size*3 bin_size*4 length(desireds_sorted)];
                        j = j+1;
                        hit_rate(j) = length(intersect(find(desireds_sorted == 1),[1:i])) / length(find(desireds_sorted == 1));
                        fa_rate(j) = length(intersect(find(desireds_sorted == 2),[1:i])) / length(find(desireds_sorted == 2));
                    end
...
%                     
                ...
                if flags.write_data_log_to_text_file==1
                    ...
                    data_log.auc_5_bins(x) = auc_overall;
                    ...
                    data_log.roc_5_bin_hits(x,:)= hit_rate;
                    data_log.roc_5_bin_fas(x,:)= fa_rate;
                end
            end
        end
    end
    ...
    if flags.write_data_log_to_text_file==1
        ...
        for q=1:x
            ...
            fprintf(fid, '%4.4f\t', data_log.auc_5_bins(q));
            fprintf(fid, '%4.0f\t', (num_trials(q)/2)); 
            fprintf(fid, '%4.4f\t', data_log.acc_percentiles(q,:));
            fprintf(fid, '\t');
             fprintf(fid, '%4.4f\t', data_log.roc_5_bin_hits(q,:));
             fprintf(fid, '\t');
             fprintf(fid, '%4.4f\t', data_log.roc_5_bin_fas(q,:));
%              fprintf(fid, '\t');
...
%              fprintf(fid, '%4.4f\t', data_log.fa_percentiles(q,:));
        ...
        fprintf(fid, '%4.4f\t', mean(data_log.auc_5_bins));
        fprintf(fid, '%4.0f\t', (mean(num_trials(q))/2)); 
        fprintf(fid, '%4.4f\t', mean(data_log.acc_percentiles,1));
        fprintf(fid, '\t');
        fprintf(fid, '%4.4f\t', mean(data_log.roc_5_bin_hits,1));
        fprintf(fid, '\t');
        fprintf(fid, '%4.4f\t', mean(data_log.roc_5_bin_fas,1));
%         fprintf(fid, '\t');
...
%         fprintf(fid, '%4.4f\t', mean(data_log.fa_percentiles,1));
    ...
    keep b subj_array condition1 condition2 nVox roi_name
