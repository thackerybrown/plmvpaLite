function []= JR_run_mvpa_v7(subj_array, condition1, condition2)
...
for b=subj_array  % allows subject ID #'s to be specified in short format
    ...
    xls_results_data_logs_dir=[expt_dir '/mvpa_results/feature_selection_tests/' condition1 '_vs_' condition2];
    
    roi_name = 'merged_AAL_ROIs_FIXED_HOLES';  % name of anatomical mask
    data_imgs_to_use = 'raw_filenames_s8mm.mat'; % .mat file containing names of all functional images
    ...
    mvpa_workspace = [expt_dir '/' subj_id '/mvpa/' subj_id '_merged_AAL_ROIs_FIXED_HOLES_s8mm.mat'];
    

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Set flags (% unless otherwise indicated: 1 = yes please, 0 = no thanks)
    flags.num_results_iter = 6; % number of times to run the entire classification process (select subset of the data and train/test classifier)
    ...
    flags.anova_p_thresh = 1;%.0008;  % p-value threshold for feature selection ANOVA (1 = DON'T PERFORM ANY FEATURE SELECTION)
    flags.GLM_anova_p_thresh = .05;
    flags.anova_nVox_thresh = 2000; % alternative to specifying p-value threshold; uses top N voxels (0 = DON'T PERFORM ANY FEATURE SELECTION)
    ...
    for n = 1: flags.num_results_iter
        ...
        if ~isempty(find(strcmp(subj_id,{'s104','s105','s108','s113','s117','119','120','121'})))
            all_regs(6,:)=0;
        end
        ...
        if flags.GLM_anova_p_thresh ~= 1
            
            uncondensed_regs_of_interest = [];
            eval(['uncondensed_regs_of_interest(1,:) = ' condnames{1} ';'])
            eval(['uncondensed_regs_of_interest(2,:) = ' condnames{2} ';'])
            subj = init_object(subj,'regressors','conds_uncondensed');
            subj = set_mat(subj,'regressors','conds_uncondensed',uncondensed_regs_of_interest);
            subj = set_objfield(subj,'regressors','conds_uncondensed','condnames',condnames);
            
            subj = create_xvalid_indices(subj,'runs');
            
            %subj = convolve_regressors_afni(subj,'conds_uncondensed','runs','afni_location','/Users/Jesse/abin/');
            
            %subj = JR_feature_select(subj,'spiral_d_hp_z_condensed','conds','runs_xval','thresh',flags.anova_p_thresh, 'statmap_funct','statmap_3dDeconvolve');
            statmap_arg.use_mvpa_ver = true;
            subj = JR_feature_select(subj,'spiral_d_hp_z','conds_uncondensed','runs_xval','thresh',flags.anova_p_thresh, 'statmap_arg',statmap_arg);
        ...
        if flags.anova_p_thresh ~= 1
            ...
            subj = JR_feature_select(subj,'spiral_d_hp_z_condensed','conds','runs_xval','thresh',flags.anova_p_thresh, 'statmap_funct','statmap_anova');
        ...
        if flags.anova_nVox_thresh ~=0
            %subj = JR_feature_select_top_N_vox_iterative(subj,'spiral_d_hp_z_condensed','conds','runs_xval','nVox_thresh',flags.anova_nVox_thresh);
            subj = JR_feature_select_top_N_vox_iterative(subj,'spiral_d_hp_z_condensed','conds','runs_xval','nVox_thresh',flags.anova_p_thresh, 'statmap_funct','statmap_RFE');
            classifier_mask = subj.masks{end}.group_name; % use group of masks created by ANOVA
        else
            classifier_mask = subj.masks{1}.name; % use original mask
        end
