function []= JR_run_mvpa_v13(subj_array, condition1, condition2, nVox)
...
for b=subj_array  % allows subject ID #'s to be specified in short format
    ...
    flags.num_results_iter = 5; % number of times to run the entire classification process (select subset of the data and train/test classifier)
    ...
    if flags.optimize_penalty_param == 1
        ...
        dir_str = [condition1 '_vs_' condition2 '_' class_args.train_funct_name(6:end) 'pen' num2str(class_args.penalty) '_' num2str(flags.anova_nVox_thresh) 'vox'];
    end
    
    if flags.generate_importance_maps == 1        
        importance_maps_dir=[expt_dir '/mvpa_results/FINAL_ROC_data/feature_selection_tests/FINAL_IMP_MAPS/' dir_str];
     ...
     if flags.generate_weight_maps == 1        
        weight_maps_dir=[expt_dir '/mvpa_results/FINAL_ROC_data/feature_selection_tests/FINAL_WEIGHT_MAPS/' dir_str];
    ...
    if flags.write_data_log_to_text_file == 1        
        xls_results_data_logs_txt_dir=[expt_dir '/mvpa_results/FINAL_ROC_data/feature_selection_tests/FINAL_CLASS_PERF/' dir_str];
    ...
    if flags.save_data_log_as_mat_file == 1
        xls_results_data_logs_mat_dir=[expt_dir '/mvpa_results/FINAL_ROC_data/feature_selection_tests/data_logs/' dir_str];
...
%                     acc_percentiles(4)=mean(abs_correct_sorted(1:ceil(num_trials(x)*.25)));
                    ...
                    for acc_bin = 1:20
                        acc_percentiles(acc_bin)= mean(abs_correct_sorted(1:ceil(num_trials(x)*bin_intervals(acc_bin))));
                    end
                    ...
                    class_counts_by_quartile_rank(1,1) = count(desireds_sorted((1:ceil(num_trials(x)*1.0)))==1);
                    class_counts_by_quartile_rank(1,2) = count(desireds_sorted((1:ceil(num_trials(x)*1.0)))==2);
                    class_counts_by_quartile_rank(2,1) = count(desireds_sorted((1:ceil(num_trials(x)*.75)))==1);
                    class_counts_by_quartile_rank(2,2) = count(desireds_sorted((1:ceil(num_trials(x)*.75)))==2);
                    class_counts_by_quartile_rank(3,1) = count(desireds_sorted((1:ceil(num_trials(x)*.50)))==1);
                    class_counts_by_quartile_rank(3,2) = count(desireds_sorted((1:ceil(num_trials(x)*.50)))==2);
                    class_counts_by_quartile_rank(4,1) = count(desireds_sorted((1:ceil(num_trials(x)*.25)))==1);
                    class_counts_by_quartile_rank(4,2) = count(desireds_sorted((1:ceil(num_trials(x)*.25)))==2);
