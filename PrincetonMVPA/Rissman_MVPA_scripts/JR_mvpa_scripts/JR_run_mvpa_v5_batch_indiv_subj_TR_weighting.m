function [subj results] = JR_run_mvpa_v5_batch_indiv_subj_TR_weighting(subj_arr, cond1, cond2, balance_per_subj_bin)
...
for b=subj_arr
    ...
    importance_maps_dir=[PAST_dir '/mvpa_results/importance_maps/' cond1 '_vs_' cond2 '_' balanced_or_unbal '_indiv_subj_posHRF_TR_con_coeff'];
    xls_results_data_logs_dir=[PAST_dir '/mvpa_results/xls_results_data_logs/' cond1 '_vs_' cond2 '_' balanced_or_unbal '_indiv_subj_posHRF_TR_con_coeff'];
...
%         spm_HRF = [.0072 .2168 .3781 .2742 .1237];  % from SPM canonical values at 1,3,5,7,and 9 sec post-stimulus
%         TR_weights = spm_HRF-mean(spm_HRF)
        ...
        all_regs_copy = all_regs; %keep a copy
        ...
        all_trials = sum(all_regs_copy,1); % vector of all trial   (patterns{4} contains fully preprocessed data)
        all_ind = find(all_trials);

%         c1_ind = find(regs_of_interest(1,:)); % cond 1 trial indices
%         c2_ind = find(regs_of_interest(2,:)); % cond 2 trial indices
%         c1_and_c2_ind = union(c1_ind,c2_ind); % all trial indices
% 
...
%         end
% 
%         for t = 1: length(all_ind)
%             all_ts(t,:) = mean(subj.patterns{end}.mat(:,all_ind(t):all_ind(t)+5),1);
%         end
% 
% 
%         % take the mean across all trials
%         c1_ts_mean = mean(c1_ts,1);
%         c2_ts_mean = mean(c2_ts,1);
%         all_ts_mean = mean(all_ts,1);

        TR1 = subj.patterns{end}.mat(:,find(all_trials)+0);
        TR2 = subj.patterns{end}.mat(:,find(all_trials)+1);
        TR3 = subj.patterns{end}.mat(:,find(all_trials)+2); % 3rd TR (4-6 sec)
        TR4 = subj.patterns{end}.mat(:,find(all_trials)+3);
        TR_diff = (TR3+TR4)/2 - (TR1+TR2)/2;
        mean_TR_diff = mean(TR_diff,2);
        pos_vox = find(mean_TR_diff>0);
        for t = 1: length(all_ind)
            all_ts_pos(t,:) = mean(subj.patterns{end}.mat(pos_vox,all_ind(t):all_ind(t)+5),1);
        end

        mean_all_ts_pos = mean(all_ts_pos,1);

        %TR_weights = [mean_all_ts_pos(1:5) - mean(mean_all_ts_pos(1:5))]
        TR_weights = [0 0 1 0 0]

        %TR_weights = [0 TR_weights(2:5) - mean(TR_weights(2:5))]



        % weight TRs of interest (to correspond with peak post-stim BOLD response)
        ...
        clear data_by_TR all_ts_pos; %clean up matlab workspace to save memory
        ...
        args.ignore_diff_size = true;  % don't warn me that the pattern I'm replacing isn't the same size as the old one
        subj = set_mat(subj,'pattern','spiral_d_hp_z_condensed',temporally_condensed_data, args);
        ...
        if flags.remove_outliers == 1  % exclude any trial with a whole-brain mean value of greater than or less than 2
            whole_brain_mean = mean(subj.patterns{end}.mat(:,active_trials),1);
            upper_outliers = find(whole_brain_mean>2);
            lower_outliers = find(whole_brain_mean<-2);
            all_outliers = union(upper_outliers,lower_outliers)  %display outlier indices to command window
            %actives_selector(all_outliers) = 0; % remove outliers for selector
        end
        ...
        display('performing second round of zscoring')
        ...
        if flags.display_performance_breakdown == 1
            ...
            if num_trials>50
                ...
                %compute ranked accuracy for each group of 50 trials, so
                %long as there are at least 20 more trials left
                for j= 1:floor((num_trials-20)/50)  
    ...
    if flags.write_data_log_to_text_file==1
        ...
        fprintf(fid, '%s\r\n', ['TR weightings = ' num2str(TR_weights)]);
