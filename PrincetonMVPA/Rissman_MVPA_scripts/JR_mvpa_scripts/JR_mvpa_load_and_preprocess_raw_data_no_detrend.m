function [subj] = JR_mvpa_load_and_preprocess_raw_data_no_detrend(subj_id, exp_name, roi_name, roi_file, raw_filenames, num_runs, num_TP_per_run)
    ...
    subj = load_analyze_pattern(subj,'spiral',roi_name,raw_filenames,'single',true);
   ...
   %subj = move_pattern_to_hd(subj, 'spiral');
    ...
    % clean up workspace
    %ubj = remove_mat(subj,'pattern','spiral');
    
    % high-pass filter the timeseries data
    subj = hpfilter_runs(subj,'spiral','runs',100,2); % remove frequencies below .01 Hz
