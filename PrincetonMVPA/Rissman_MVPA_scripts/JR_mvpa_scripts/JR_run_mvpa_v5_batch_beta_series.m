...
for b=subj_arr
    ...
    importance_maps_dir=[PAST_dir '/mvpa_results/importance_maps/' cond1 '_vs_' cond2 '_' balanced_or_unbal '_beta_series'];
    xls_results_logs_dir=[PAST_dir '/mvpa_results/xls_results_logs/' cond1 '_vs_' cond2 '_' balanced_or_unbal '_beta_series'];
    ...
    %mvpa_workspace = [PAST_dir '/' subj_id '/mvpa/' subj_id '_merged_AAL_ROIs_beta_series_8mm_smoothing.mat'];
    ...
    for x = 1: num_results_iter
        ...
        if ~exist('mvpa_workspace')
            ...
            data_imgs_to_use = [mvpa_dir '/beta_series_filenames.mat'];
            load(data_imgs_to_use); %loads predefined cell array called raw_filenames into memory

            num_runs = length(raw_filenames)/num_trials_per_run; %calculate the number of runs (allows script to work flexibly for subjects with missing runs)
            ...
            subj = load_analyze_pattern(subj,'betas',roi_name,raw_filenames,'single',true);
            ...
            if flags.save_workspace == 1
                save_cmd = ['save ' mvpa_dir '/' subj_id '_' roi_name '_beta_series_8mm_smoothing.mat'];
                eval(save_cmd);
            end
        else
        ...
        data_imgs_to_use = 'raw_filenames_beta_series_s8mm_wa.mat';
        ...
        % strip nan voxels from mask

        nan_vox = find(isnan(subj.patterns{end}.mat(:,1)));
        subj.patterns{end}.mat(nan_vox,:)=[];
        subj.patterns{end}.matsize = size(subj.patterns{end}.mat);
        mask_vox = find(subj.masks{1}.mat);
        mask_nans = mask_vox(nan_vox);
        subj.masks{1}.mat(mask_nans)=0;
        subj.masks{1}.nvox = count(subj.masks{end}.mat==1);
        ...
        upper_outliers = find(z_mean_across_voxels> 2.5);
        lower_outliers = find(z_mean_across_voxels< -2.5);
        ...
        if anova_p_thresh ~= 1
            subj = feature_select(subj,'betas','conds','runs_xval','thresh',anova_p_thresh);
        ...
        [subj results{x}] = cross_validation(subj,'betas','conds','runs_xval',classifier_mask,class_args);
