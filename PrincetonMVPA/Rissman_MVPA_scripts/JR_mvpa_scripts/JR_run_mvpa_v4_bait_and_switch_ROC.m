function [subj results] = JR_run_mvpa_v4_bait_and_switch_ROC(subj_array)

tic

for a=1:length(subj_array)
    b = subj_array(a);
    if b<10
    ...
    mvpa_workspace=strcat('/Users/Jesse/fMRI/data/PAST/fMRI/' , subj_id, '/mvpa/', subj_id, '_merged_AAL_ROIs_FIXED_HOLES_wa.m.mat');
...
%class_args.test_funct_name = 'test_smlr';
    ...
    if flags.equate_number_of_trials_in_cond_1_and_2 == 1
        ...
        cond1_trials = find(regs_bait(1,:));
        cond2_trials = find(regs_bait(2,:));
...
%     subj.patterns{end}.mat(:,active_trials) = zscore(subj.patterns{end}.mat(:,active_trials)')';
    ...
    classifier_mask = subj.masks{2}.name;
...
%             end
        ...
        switch_overall_accuracy = mean(switch_correct_vector)
        switch_overall_hit_rate = mean(switch_correct_vector(switch_desireds_vector==1))
        switch_overall_fa_rate = 1-mean(switch_correct_vector(switch_desireds_vector==2))
        switch_overall_d_prime = norminv(switch_overall_hit_rate)-norminv(switch_overall_fa_rate)

        % sort by absolute value of classifier "confidence"
        [switch_abs_sorted_diffs switch_abs_ind] = sort(abs(switch_acts_diff_vector),2,'descend');
        switch_abs_correct_sorted = switch_correct_vector(switch_abs_ind);
        switch_abs_desireds_sorted = switch_desireds_vector(switch_abs_ind);
        ...
        if num_trials>50
            ...
            % classification_accuracy_by_resp (for full classification only)
            for b = 1:10
                classification_accuracy_by_resp(b) = mean(switch_correct_vector(find(condensed_regs_all(b,active_trials))));
                mean_acts_diffs_by_resp(b) = mean(switch_acts_diff_vector(find(condensed_regs_all(b,active_trials))));
                number_of_trials_per_bin(b) = length(find(condensed_regs_all(b,active_trials)));
            end
        ...
        if flags.plot_ROC_curve == 1
            ...
            [switch_sorted_diffs ind] = sort(switch_acts_diff_vector,2,'descend');
            switch_correct_sorted = switch_correct_vector(ind);
            switch_desireds_sorted = switch_desireds_vector(ind);

            % create continuous ROC function
%             for i = 1:length(switch_sorted_diffs);
%                 switch_hit_rate(i) = length(switch_correct_sorted(intersect(find(switch_desireds_sorted == 1),[1:i]))) / length(find(switch_desireds_sorted == 1));
%                 switch_fa_rate(i) = length(switch_correct_sorted(intersect(find(switch_desireds_sorted == 2),[1:i]))) / length(find(switch_desireds_sorted == 2));
%             end
            
             num_bins = 7;
                for j= 1:num_bins

                    hit_count(j) = length(find(switch_desireds_sorted(1:ceil(j*num_trials/num_bins))==1));
                    fa_count(j) = length(find(switch_desireds_sorted(1:ceil(j*num_trials/num_bins))==2));
                end


                hit_rate = hit_count / length(find(switch_desireds_sorted == 1));
                fa_rate = fa_count / length(find(switch_desireds_sorted == 2));
                
                figure
                [Ro,Rn,dp,crit,SSE,pts,flag]=dpsd(hit_rate,fa_rate,'trim','print')
