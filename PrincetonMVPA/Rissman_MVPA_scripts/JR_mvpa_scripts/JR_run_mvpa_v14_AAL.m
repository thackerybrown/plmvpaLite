function []= JR_run_mvpa_v14_AAL(subj_array, condition1, condition2, nVox)
...
for b=subj_array  % allows subject ID #'s to be specified in short format
    ...
    if flags.optimize_penalty_param == 1
        dir_str = ['AAL_80_ROIs_' condition1 '_vs_' condition2 '_' class_args.train_funct_name(6:end) 'OPTIMAL_pen_' num2str(flags.anova_nVox_thresh) 'vox'];
    else
        dir_str = ['AAL_80_ROIs_' condition1 '_vs_' condition2 '_' class_args.train_funct_name(6:end) 'pen' num2str(class_args.penalty) '_' num2str(flags.anova_nVox_thresh) 'vox'];
    end
...
%            display('shuffling runs vector')
        ...
        AAL = spm_vol(roi_img_with_labels);
        AAL_data = spm_read_vols(AAL);
        
         voxel_inds = find(subj.masks{end}.mat); %get mask voxel indices
         
        AAL_values_within_whole_brain_mask = AAL_data(voxel_inds);
        
        %num_AAL_ROIs = max(AAL_values_within_whole_brain_mask);
        
        roi_list = [1:50, 53:56,59:62,67:74,77:90]; num_AAL_ROIs = length(roi_list);
        for roi_idx = 1:num_AAL_ROIs
            aa = roi_list(roi_idx);          
            aa_vox_inds = find(AAL_values_within_whole_brain_mask==aa);
            AAL_only_data_matrix(roi_idx,:)= mean(temporally_condensed_data(aa_vox_inds,:),1);
        end
        
        whole_mask_data_matrix =  zeros(size(temporally_condensed_data));
        whole_mask_data_matrix(1:num_AAL_ROIs,:)=AAL_only_data_matrix;
        
        subj.masks{1}.mat = zeros(subj.masks{1}.matsize);
        subj.masks{1}.mat(1:num_AAL_ROIs)=1;
        ...
        subj = set_mat(subj,'pattern','spiral_d_hp_z_condensed',whole_mask_data_matrix,'ignore_diff_size',true);
