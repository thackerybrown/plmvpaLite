function [subj] = JR_extract_voxel_importance_values(subj,results,results_IW,varargin)

% adapted from MVPA toolbox script interpret_weights.m

% Based on comments from Ken Norman on the MVPA mailing list (and from
% recently published papers from his lab by McDuff et al., and Johnson, et al.)

% The net contribution of a voxel to activating a task unit
...
for i=1:nTimes
    for j=1:nRuns
        ...
        curWts{1}=results_IW{i}.iterations(j).scratchpad.net.IW{1}(1,:)'; % JR: get weights for output unit 1
        curWts{2}=results_IW{i}.iterations(j).scratchpad.net.IW{1}(2,:)'; % JR: get weights for output unit 2

        %      if strcmp(args.type_canon,'pos')
        % 	curWts(find(curWts<0))=0;
        %       end
        curCanonical{1} = iteration{j}.canonical(:,1); % JR: get mean activity for output unit 1
        ...
        importance{j}.map(:,1) = importance{j}.map( :,1) + curImp{1};
        importance{j}.map(:,2) = importance{j}.map( :,2) + curImp{2};
    end
end
