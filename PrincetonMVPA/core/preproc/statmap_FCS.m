function [subj] = statmap_FCS(subj,data_patname,regsname,selname,new_map_patname,extra_arg)
...
hist = sprintf('Created by statmap_FCS');
subj = add_history(subj,'pattern',new_map_patname,hist);

created.function = 'statmap_FCS';
...
for j=1:nVox
  ...
  m1 = mean(pat(j,dataIdx(find(groups==1))));
  m2 = mean(pat(j,dataIdx(find(groups==2))));
  v1 = var(pat(j,dataIdx(find(groups==1))));
  v2 = var(pat(j,dataIdx(find(groups==2))));
  FCS = (m1 - m2)^2 / (v1+v2);  
  p(j) = FCS;
end   
