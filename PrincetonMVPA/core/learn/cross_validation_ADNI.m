function [subj results] = cross_validation_ADNI(subj,patin,regsname,selname,maskgroup,class_args,varargin)
% Modified by TIB from 'cross_validation' to handle cases where existing
% pattern matrix precludes need for masking
%
...
%results.total_perf = nanmean(store_perfs,2); %JR version of total_perf
results.total_perf = mean(store_perfs,2);       %modified by MU 7/15/13 due to incompatibility with spm8/matlab 2011b
