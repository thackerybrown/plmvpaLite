function [subj] = load_matrix_pattern_2D(subj,new_patname,maskname,mat)

% Loads a 2D matrix into the SUBJ structure as a pattern
% Written by TIB for bypassing SPM pattern loading processes when an
% existing features*patterns 2D mat exists
%
...
vDims = size(mat);
if length(vDims)~=2
  % if you're feeding in a 3-4D matrix then call load_matrix_pattern instead
  error('Your input matrix must be 2D (features,time)');
end
