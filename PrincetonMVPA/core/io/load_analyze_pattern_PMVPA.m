function [subj] = load_analyze_pattern_PMVPA(subj,new_patname,maskname,filenames,varargin)
...
for h = 1:nFiles % start looping thru the files being used.
  ...
  for i = 1:m
     ...
     % Enforce mask size
     if ~all(curvol.dim == size(maskvol))%AG variant - for some reason curvol.dim doesn't exist - something about how vol is being read above
     %if ~all(curvol{1}.dim == size(maskvol))
     ...
     [Vdata] = spm_read_vols(curvol);%AG variant - again, curvol isn't correct
     %[Vdata] = spm_read_vols(curvol{1});
