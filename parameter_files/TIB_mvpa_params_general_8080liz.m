function [S idxTr idxTe par]= TIB_mvpa_params_general_8080(subj_id, task, TRsperRun, imgtype)
% updated for PSYCH 8080, NEUR+PSYCH 4803 FA 2023

% This file establishes parameters for mvpa analysis, which are then used
% by TIB_run_mvpa_general.m. Run that script, with a few input arguments,
...
% string
% <task> - a signifier of the experimental task phase being analyzied (e.g., 'goals'
% or 'plan'). A placeholder can be used when this is non-sensical for your
% study... just be sure to update code points below which reference a
% particular string input for the <task> argument so that the input matches
% possible strings this script is looking for.
% <TRsperRun> - when your subject has data from multiple scan
% runs/sessions, the code needs to know how long each one is. Since the
% original use of this code was for fMRI, the naming scheme refers to "TRs"
% (# of BOLD images) acquired each experiment session. If you are not using
% fMRI data, consider running the scrips with the "existpatmat" flag on,
% and providing a matlab matrix of the data you want to classify. Could be
% EEG data, behavioral data, anything you want. In such a scenario, the
% "TRsperRun" input should simply be the number columns of data in your
% matlabe input file (this is what true fMRI BOLD scans get converted to
% anyway in the case of fMRI, so you are just scipping the image reading
% step)
% <imgtype> - 'raw' or 'betas' - this argument defines whether you have one
% multiple images which you might want to collapse into one pattern of
% interest ('raw'; taking multiple "TRs" of data and averaging them using
% rules defined below) or if each "TR" should actually already be
% considered a pattern ready for classification ('betas')
...
% live
S.sbasepath = '/home/thackery/Documents/work/mvpa_sample_data/';
