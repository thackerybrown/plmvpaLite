function [S idxTr idxTe par]= TIB_mvpa_params_general_PIB(subj_id, task, TRsperRun, imgtype)
...
S.datafile = 'feature_set_pibshort.mat'; % added for ADNI neuropsych study. Replaces MRI image (e.g., nii) files with an existing matrix of pattern data

% ~~~ HOW MANY *single trial betas* are there? If data input type is 'raw', number is not used (can be ignored)
S.stbetacount = 127; % NOTE: the code assumes all single trial betas of potential interest are contiguous in the model. If multi-event betas are inteleaved in the .mat model structure, this will need more editing.
...
S.trainTask = 'posvsneg';%Circmaze - 'goals' or 'plan'
...
S.testTask = 'posvsneg';%Circamze - 'goals' or 'plan'
...
S.xvaltype = 'nf';
S.nFolds = 127; % number of cross validation iterations - only used for nFold (as opposed to run-by-run loo)
...
S.trainonsfnamebetas = '_names_set_pibshort';
S.testonsfnamebetas = '_names_set_pibshort';
...
S.class_args.nVox = 3; % number of voxels to select with feature selection e.g. [1000 5000 10000]
...
if strcmp(S.trainTask,'posvsneg')
    S.onsetsTrainDir = [S.mvpa_dir];%directory containing onsets.mat or betas_idx.mat file to be loaded in
    S.condsTrain = {{' pibpos'}  {'pibneg'}} ;%corresponds to the names in the onsets.mat or betas_idx.mat files. This is used to select what is being compared with what.
...
elseif strcmp(S.inputformat, 'betas')
    S.preprocPatName = 'betas_z';%'betas';%;%use betas_z if z-scoring betas
end
