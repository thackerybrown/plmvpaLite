function [S idxTr idxTe par]= TIB_mvpa_params_EOG(subj_id, task, TRsperRun, imgtype)
...
S.datafile = 'EDITED-S1T1-BYB_Recording_2018-11-14_13.57.30_matrix_merged_1vs6.mat'; % added for ADNI. Replaces MRI image files with an existing matrix of pattern data
...
S.trainTask = 'OnevsTwo';%which ptcpt categories to train on
S.testTask = 'OnevsTwo';%which ptcpt categories to test on
...
S.stbetacount = 89; % NOTE: the code assumes all single trial betas of potential interest are contiguous in the model. If multi-event betas are inteleaved in the .mat model structure, this will need more editing.
...
elseif strcmp(S.inputformat, 'betas')
    S.onsets_filename = ['EDITED-S1T1-BYB_Recording_2018-11-14_13.57.30_matrix_merged_1vs6.mat'];%['onsets_' S.subj_id '_ADNI1'];
    S.onsets_filename_tr = ['EDITED-S1T1-BYB_Recording_2018-11-14_13.57.30_matrix_merged_1vs6.mat'];%['onsets_' S.subj_id '_ADNI1'];
    S.onsets_filename_tst = ['EDITED-S1T1-BYB_Recording_2018-11-14_13.57.30_matrix_merged_1vs6.mat'];%['onsets_' S.subj_id '_ADNI1'];
...
elseif strcmp(S.inputformat, 'betas')
    S.mvpa_dir = [par.subdir '/TimeMarkerCorrectedData/'];
end
...
if strcmp(S.trainTask,'OnevsThree')
    S.onsetsTrainDir = [S.mvpa_dir];%directory containing onsets.mat or betas_idx.mat file to be loaded in
    S.condsTrain = {{'1'}  {'3'}} ;%corresponds to the names in the onsets.mat or betas_idx.mat files. This is used to select what is being compared with what.
...
elseif strcmp(S.trainTask,'OnevsTwo')
    S.onsetsTrainDir = [S.mvpa_dir];%directory containing onsets.mat or betas_idx.mat file to be loaded in
...
elseif strcmp(S.testTask,'OnehundredvsThreehundred')
    S.onsetsTestDir =[S.mvpa_dir];%directory containing onsets.mat or betas_idx.mat file to be loaded in
    S.condsTest = {{'100'} {'300'}};
    S.nwayclass = num2str(numel(S.condsTest));%stores the number classification dimensions just for reference (i.e. is this a 5-way or a 2-way/binary classification?)
    S.TestRuns = par.scansSelect.(par.task).loc;
...
elseif strcmp(S.testTask,'OnehundredvsTwohundred')
    S.onsetsTestDir =[S.mvpa_dir];%directory containing onsets.mat or betas_idx.mat file to be loaded in
    S.condsTest = {{'100'} {'200'}};
    S.nwayclass = num2str(numel(S.condsTest));%stores the number classification dimensions just for reference (i.e. is this a 5-way or a 2-way/binary classification?)
    S.TestRuns = par.scansSelect.(par.task).loc;
